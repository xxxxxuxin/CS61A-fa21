...
# how does this func get the right expr ? 
# ans: this is dealt by lexical analysis
...
def do_lambda_form(expressions, env):
    ...
    body = nil
    body_list = []
    expr = expressions.rest.map(lambda x:x)
    while expr != nil:
        body_list.append(expr.first)
        expr = expr.rest
    
    for x in body_list[::-1]:
        body = Pair(x, body)
...
def do_enum_form(expressions, env):
    #print("DEBUG: ", expressions)
    validate_form(expressions, 1, 1)

    sc_list = scheme_eval(expressions.first, env)
    ...
    if sc_list == nil:
        ...
        i = 0
        while sc_list != nil:
            enum_list = Pair(Pair(i, Pair(sc_list.first, nil)), enum_list)
            sc_list = sc_list.rest
            i += 1
        reverse_list = nil
        while enum_list != nil:
            reverse_list = Pair(enum_list.first, reverse_list)
            enum_list = enum_list.rest
        return reverse_list

def do_merge_form(expressions, env):
    validate_form(expressions, 3, 3)
    compare = scheme_eval(expressions.first, env).py_func
    first_list = scheme_eval(expressions.rest.first, env)
    second_list = scheme_eval(expressions.rest.rest.first, env)
    return merge_helper(first_list, second_list, compare)


def merge_helper(s1, s2, compare):
    if s1 == nil:
        return s2
    elif s2 == nil:
        return s1
    else:
        sorted_sublist = merge_helper(s1.rest, s2.rest, compare)
        if compare(s1.first, s2.first):
            return Pair(s1.first, Pair(s2.first, sorted_sublist))
        else:
            return Pair(s2.first, Pair(s1.first, sorted_sublist))
