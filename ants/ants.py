...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not near_place.is_hive:
            if near_place.bees and self.min_range <=distance <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        cause_damage = amount + self.damage if amount >=self.health else amount
        
        for bee in bees:
            #original_bee = self.place.bees[self.place.bees.index(bee)]
            bee.reduce_health(cause_damage)
        # super().reduce_health(amount) alternatively
...
class WallAnt(Ant):
    name = 'Wall'
    #health = 4 # health is an instance attribute !
    food_cost = 4
    implemented = True
...
class TankAnt(ContainerAnt):
    ...
    def action(self,gamestate):
        ...
        for bee in bees:
            original_bee = self.place.bees[self.place.bees.index(bee)]
            original_bee.reduce_health(self.damage)
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        """
        # BEGIN Problem 12
        if not gamestate.queen_dep:
            gamestate.queen_dep = True
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        if self.place.ant and self.place.ant.blocks_path==True:
            return True
        return False
    ...
    def action(self, gamestate):
        ...
        if self.slow_turn :
            if self.health > 0 and gamestate.time%2==0 and destination is not None:
        ...
        if self.scared_turn and not(self.slow_turn and gamestate.time%2):
            self.scared_turn -= 1
            #print("DEBUG:" , self.scared_turn)
        if self.slow_turn:
            self.slow_turn -= 1
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        scope = self.place
        distance = 0
        while not scope.entrance.is_hive:
            #print("DEBUG:" , insects)
            insects[scope.ant] = distance
            for bee in scope.bees:
...
class GameState:
    ...
    def __init__(self, strategy, beehive, ant_types, create_places, dimensions, food=2):
        ...
        queen_dep -- a bool indicates whether a QueenAnt has been deployed
        """
        self.time = 0
        self.food = food
