...
def picky_piggy(score):
    ...
    if score==0:
        ...
        return int(1/7 * pow(10,score%6))%10
    else: 
        return 7
    # END PROBLEM 2
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        who_arg = [score1, score0, strategy1] if who else [score0, score1,strategy0]
        opponent_score=who_arg[1]
        num_rolls=who_arg[2](who_arg[0],who_arg[1])
        #print("DBUG", who, score0, score1, who_arg[0], opponent_score, num_rolls)
        who_arg[0] += take_turn(num_rolls,opponent_score,dice,goal)
        #print("DBUG", who_arg[0])
        who_arg[0] += hog_pile(who_arg[0],who_arg[1])
        if who:
            score1=who_arg[0]
            #print("DBUG1:", score0, score1)
        else:
            score0=who_arg[0]
            #print("DBUG0:", score0, score1)
        who = next_player(who)
    # END PROBLEM 5
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0,score1):
        #print("DBUG:", last_score)
        focus = score1 if who else score0
        gain = focus - last_score
        if  gain > running_high:
            print(gain, "point(s)! That's a record gain for Player %s!" % who)
            return announce_highest(who, focus, gain)
        else:
            return announce_highest(who, focus, running_high)
...
def hog_pile_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    delta = picky_piggy(opponent_score) 
    if score + delta == opponent_score:
        return 0
    else:
