...
def count_coins(change):
    ...
    True
    """
    def count_with_cond(change, value):
    	if change==0:
    		return 1
    	elif change<0:
    		return 0
    		...
    		return 0
    	with_value = count_with_cond(change-value,value) 
    	without_value = count_with_cond(change,descending_coin(value))
    	return with_value + without_value

    return count_with_cond(change, 25)
...
def make_anonymous_factorial():
    ...
    return  (lambda f: lambda x: f(f, x))(lambda f , k: k if k == 1 else k * f(f, k-1) )
